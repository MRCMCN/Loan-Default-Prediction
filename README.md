...
- Five columns are dropped upfront: an identifier, a constant year, and three fields where a single category covers over 99.5% of the observations. Seven more are dropped as contaminated, leaving 21 inputs.
...
- In Gini terms, the convention in credit scoring, the gap is 0.576 against 0.460 — a 25% relative gain for the challenger. Real, but small enough that the interpretability trade-off is genuine in a regulatory context where explainability is a requirement rather than a preference.
- Train-to-test degradation is small for both models (GB: AUC 0.809 → 0.788), indicating no substantial overfitting.

**Operating cutoff**

- At the default 0.5 threshold the champion catches 36% of defaults (1,331 of 3,664).
- At the KS-optimal cutoff estimated on validation (0.264) it catches 62%, at the cost of flagging 2,188 good applications instead of 461. F1 rises from 0.488 to 0.562, accuracy falls from 0.812 to 0.760.
